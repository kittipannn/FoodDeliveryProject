...
public class MotorcycleControl : MonoBehaviour
{
    ...
    public class BikeSetting
    {
        ...
        //น่าจะเป็นตัวเพิ่มมุมกล้อง
        public List<Transform> cameraSwitchView;


        public Transform MainBody; //ใส่ตัวรถ
        public Transform bikeSteer;//ใส่แฮนด์จับ
        ...
        public float springs = 35000.0f; //สปริง น่าจะไว้ใช้กับ springjoint
    ...
    void Update()
    {
        ...
        if (bikeSetting.bikeSteer)
            bikeSetting.bikeSteer.localRotation = SteerRotation * Quaternion.Euler(0, 0, wheels[0].collider.steerAngle); // this is 90 degrees around y axis
    ...
    void FixedUpdate()
    {
        ...
        foreach (WheelComponent w in wheels)
        {
            ...
            if (col.GetGroundHit(out hit) && (w == wheels[1] || (w == wheels[0] && Wheelie == 0)))
            {
                ...
                Debug.Log(col.GetGroundHit(out hit));
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawCube(Vector3.up / 2f, new Vector3(0.5f, 0.8f, 1f));
        Gizmos.DrawSphere(bikeSetting.shiftCentre, 0.1f);
